...
read the result in two ways. It also includes a browser-side video conversion
tool for turning MP4 and MOV files into MP3 audio that can be downloaded or
sent straight into the transcription flow:
...
The video-to-MP3 flow is separate from transcription:

1. Switch to `Video to MP3`
2. Upload an MP4 or MOV file
3. Wait for client-side conversion to finish
4. The generated MP3 is selected automatically in the `Transcribe` tab
5. Download the MP3 there if needed
...
- TypeScript
- Tailwind CSS 4
- ffmpeg.wasm for browser-side video-to-MP3 conversion
